...
- Git 提交统一以 `doc/通用模板/Git提交描述模板.md` 为模板主源；提交前先同步远端主线并处理冲突。
- 提交标题必须使用 `feat|fix|docs: 具体标题（AI生成提交）` 格式；提交描述必须固定包含 `AI辅助：true`、`AI工具：Trae`、`1. 改了什么`、`2. 为了解决什么问题`，且内容必须具体可审计。
