...
- 开工前必须先查询项目图谱：`doc/项目勘查报告/02-AI工具检索用项目图谱.md`。
...
## 3. 图谱预读规则
- AI 工具进入 `d:\Probject\Gjj` 后，默认先读：
  - `README.md`
  - `doc/README.md`
  - `doc/项目勘查报告/02-AI工具检索用项目图谱.md`
- 先根据项目图谱判断当前任务属于：`prod`、`jiaxing`、`linyi`、`zaozhuang`、`wenzhou`、`v3`、前端项目、后端项目，禁止直接全仓检索。
- 先按图谱选 1 到 3 个目标目录，再搜文件名，再搜关键词；不要默认全量扫描 `doc/操作记录/`、`doc/技能库/`、`doc/评审记录/`、`doc/设计文档/`。
- 若任务要求修改业务源码，必须先根据图谱确认逻辑项目，再确认真实源码目录；不得把当前文档仓误当成源码仓。
...
请严格遵循以下要求：
1. 开工前先读取 `README.md`、`doc/README.md`、`doc/项目勘查报告/02-AI工具检索用项目图谱.md`。
2. 先根据项目图谱判断任务属于前端、后端、前后端联动，及其归属项目空间（如 `prod`、`jiaxing`、`linyi`、`zaozhuang`、`wenzhou`、`v3`）。
3. 先判断涉及的业务模块、代码层级与目标目录。
4. 如任务属于分析、设计、评审，必须落盘到对应 `doc` 目录。
5. 如属于中大型需求或重构，必须先产出 SDD。
6. 后端必须遵循 DDD 边界、`R<T>`、DTO、BizException、Feign 降级规范。
7. 前端必须遵循现有分层、复用公共组件、统一请求封装与 `Vue 2 Options API` 风格。
8. 编译、打包、发布前必须校验工具链版本；后端 Java 默认 `vfox use java@17.0.2+8`。
9. 输出必须包含：目标项目与模块、代码放置位置、修改文件清单、实现思路、风险与影响范围、验证步骤与结果、对应文档路径。
