...
- 开工前必须先读项目图谱：`doc/项目勘查报告/02-AI工具检索用项目图谱.md`。
- 若任务涉及 `prod`、`jiaxing`、`linyi`、`zaozhuang`、`wenzhou`、`v3`，先按图谱判断逻辑项目空间，再定位真实源码路径或文档入口。
...
要求：
1. 先读取 `README.md`、`doc/README.md`、`doc/项目勘查报告/02-AI工具检索用项目图谱.md`。
2. 先明确目标项目、目标模块、目标层级、目标目录。
3. 中大型需求先产出 SDD 文档，再进入编码。
4. 保持分层清晰，返回统一 `R<T>`，入参与出参使用 DTO。
5. Controller 补齐校验、日志与接口注解。
6. 简单 CRUD 优先 MyBatis-Plus，复杂查询放 XML。
7. Feign 必须提供降级，且降级不得 `return null`。
8. 输出：修改文件清单、实现说明、风险点、验证步骤、文档路径。
```
...
输出：
1. 开工前已读取项目图谱，并确认目标项目与目录
2. 根因定位（controller/service/sql/feign/job）
3. 修复方案（最小改动）
4. 风险评估（兼容性/性能/一致性）
5. 回归用例（正常/异常/边界）
```
