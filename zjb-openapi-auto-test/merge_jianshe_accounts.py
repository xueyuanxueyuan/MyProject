...
用户提供 JPG 截图，包含 2 条对私账户（账号/户名/证件号/手机号）。

原文件 `建行测试数据预埋收集表-建设银行.xlsx` 已存在，先读原文档、备份、
...
SRC = Path(r"D:\文档\工作文档\嘉兴住建部测试数据\建行测试数据预埋收集表-建设银行.xlsx")

# 从 JPG 截图读出的新增数据
# 表头：序号, 客户号*, 账号*, 卡号, 证件类型, 证件号码*, 姓名*, 账户类型, 开户行, 联行号, 多方协议号, 手机号, 开户日期, 备注
NEW_PRIVATE = [
    # 表头：序号, 客户号*, 账号*, 卡号, 证件类型, 证件号码*, 姓名*, 账户类型, 开户行, 联行号, 多方协议号, 手机号, 开户日期, 备注
    ["", "6217001180074393565", "", "身份证", "513324198507047120", "曾雪松", "对私", "建设银行", "", "", "18947611531", "", ""],
    ["", "6217001180074393573", "", "身份证", "511903199509199578", "于天翊", "对私", "建设银行", "", "", "13285101621", "", ""],
]
...
def main() -> None:
    ...
    # 填写说明：追加一行备注
    shuoming = rows["填写说明"]
    shuoming.append(["2026-09-14 JPG 截图补充", "对私+2（曾雪松/于天翊）。"])

    sheets = [(name, rows[name]) for name in rows.keys()]
    write_xlsx(SRC, sheets)
    print(f"对私追加 {len(NEW_PRIVATE)} 条完成。")
