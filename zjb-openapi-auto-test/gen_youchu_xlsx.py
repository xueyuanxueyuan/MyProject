#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成邮储银行测试数据文档（新建，原路径下本不存在，无丢数据风险）。
表结构镜像已认可的「银行测试数据收集表-浙江农信.xlsx」：
...
邮储银行代码 313009 来自 doc 操作记录银行代码对照表（313009=邮储银行），运行时用 --bank-code 313009 传入 yhdm。
"""
...
OUT = Path(r"D:\文档\工作文档\嘉兴住建部测试数据\银行测试数据收集表-邮储银行.xlsx")
...
# 2 对私（证件类型 01=身份证），3 对公（证件类型 08=统一社会信用代码，USCC 暂空）
PRIVATE_ROWS = [
    ["1", "", "6217991000107199783", "", "01", "500113196012314172", "测擎测", "对私", "邮储银行", "", ""],
    ["2", "", "6217991000107073863", "", "01", "511823196012312345", "测君", "对私", "邮储银行", "", ""],
]
PUBLIC_ROWS = [
    ["1", "", "951013013000080965", "五庄观人参果期货交易所", "对公", "邮储银行", "313009", "", "", "08", "", ""],
    ["2", "", "951019013000118651", "女儿国子母河试管婴儿中心", "对公", "邮储银行", "313009", "", "", "08", "", ""],
    ["3", "", "951013013000092245", "天庭凌霄宝殿结算中心", "对公", "邮储银行", "313009", "", "", "08", "", ""],
]
CENTER_ROWS = [
    ["1", "", "", "", "邮储银行", "", "专户", ""],
]
...
if __name__ == "__main__":
    ...
    sheets = [
        ("填写说明", [["公积金接口测试数据收集表（邮储银行）"],
                   ["邮储银行代码 313009。带 * 字段为必填。对公账户统一社会信用代码暂为空，需补充后才能发通对公签约/交易。"]]),
